...
//           <p className="text-center text-xs text-gray-300 pb-2">Rose Routine v1.0 · Made by Mukku ❤️</p>
//         </div>
//       </div>
//       <BottomNav />
//     </PageWrapper>
...
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { MdLogout, MdDeleteForever, MdCameraAlt } from 'react-icons/md';
import { FaTrophy, FaFire, FaStar } from 'react-icons/fa';
import { useApp } from '../context/AppContext';
import { clearAllData, exportDataAsJSON, exportDataAsCSV } from '../utils/storage';
...
export default function Profile() {
  const navigate = useNavigate();
  const { user, updateUser, timetable, streak, stats, settings, updateSettings, logout } = useApp();
  const fileRef = useRef();
  ...
  const statCards = [
    { label: 'Day Streak', value: streak.current, icon: <FaFire className="text-orange-500" />, sub: `Best: ${streak.best}` },
    { label: 'Avg Score', value: `${stats.weeklyAvg}%`, icon: <FaStar className="text-yellow-400" />, sub: 'This week' },
    { label: 'Tasks Done', value: stats.totalTasksCompleted, icon: <FaTrophy className="text-rose-primary" />, sub: 'Total' },
  ];
    ...
    <PageWrapper>
      <div className="min-h-screen bg-bg pb-28">
        ...
        <div className="px-5 space-y-4">
            ...
            <div className="glass-card overflow-hidden">
              {menuItems.map((item) => (
                <button
                  key={item.label}
                  onClick={item.action}
                  className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-rose-50 transition-all border-b border-gray-50 last:border-0 text-left"
                >
                  <span className="text-xl w-7">{item.icon}</span>
                  <span className="flex-1 text-sm font-semibold text-gray-700">{item.label}</span>
                  <span className="text-gray-300 text-lg">›</span>
          ...
          {/* Danger zone */}
          <div>
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-1">Danger Zone</p>
            <div className="glass-card overflow-hidden">
              <button
                onClick={handleReset}
                className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-red-50 transition-all border-b border-gray-50 text-left"
              >
                <MdDeleteForever className="text-red-400 text-xl" />
                <span className="text-sm font-semibold text-red-400">Reset All Data</span>
              </button>
          ...
          <p className="text-center text-xs text-gray-300 pb-2">Rose Routine v1.0 · Made with 🌸</p>
        </div>
      </div>
      <BottomNav />
    </PageWrapper>
